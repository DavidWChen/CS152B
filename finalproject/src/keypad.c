...
/* Definitions for peripheral KEYPAD */
#define XPAR_KEYPAD_BASEADDR 0x81400000
#define XPAR_KEYPAD_HIGHADDR 0x8140FFFF
#define XPAR_KEYPAD_DEVICE_ID 1
#define XPAR_KEYPAD_INTERRUPT_PRESENT 0
#define XPAR_KEYPAD_IS_DUAL 0
...
u32 keypad() {
	...
	PmodKYPD keypad;
	keypad.GPIO_addr = XPAR_KEYPAD_BASEADDR;
	KYPD_begin(&keypad, keypad.GPIO_addr); // init keypad
	u8 keytable[16];
	u8 k;
	for (k = 0; k < 16; k++){
		keytable[k] = k;
	}
	KYPD_loadKeyTable(&keypad, keytable); // load key table
	...
	while (1) {
		...
		key = KYPD_getKeyPressed(&keypad, keystate, cptr);
